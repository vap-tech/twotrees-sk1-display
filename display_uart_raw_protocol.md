...
Важно: файл содержит не только подтвержденные кадры штатного протокола, но и команды из probe/debug-демки. Такие строки надо проверять на живом HMI перед использованием в клиенте.
...
| `70 61 67 65 20 35 36 ff ff ff` | `page 56` | **Не подтверждено / не использовать.** Команда пришла из ранней probe-демки `show_error_page`, но текущий дисплей `page 56` не принимает. Для результата ошибки печати использовать `page 77` + `print_done_flag=0`. |
| `74 30 2e 74 78 74 3d 22 3c 74 65 78 74 3e 22 ff ff ff` | `t0.txt="<text>"` | **Не подтверждено для page 56.** Осталось от ранней probe-демки `show_error_page`; без валидной страницы 56 не является рабочим error-render pipeline. |
...
|---|---|---|
| `0x91` | Сигнал инициализации дисплея; приходит одиночным байтом без `ff ff ff`, в том числе после краткой потери питания. |
